...
void keyPressed () {
  if (char(keyCode) == 'Q') {isCatMoving = true; }
if (char(keyCode) == 'P') {isDogMoving = true; }
}
  
  void keyReleased() {
     if (char(keyCode) == 'Q') isCatMoving = false;
     if (char(keyCode) == 'P') isDogMoving = false;
  }
...
//    You can use the image provided (space), or change it to something else.
  ...
  if (isCatMoving) {moveRacer1();}
  if (isDogMoving) {moveRacer2();}
 // isCatMoving = false;
 // isDogMoving = false;
// 4. DRAW the RACERS
...
void moveRacer1(){
  racer1X = racer1X +(int) random(10);
}

void moveRacer2(){
  racer2X = racer2X +(int) random(10);
}
